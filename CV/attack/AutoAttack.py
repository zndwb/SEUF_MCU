...
def apgd_train(model, x, y, norm, eps, n_iter=10, mean=torch.tensor(np.array([125.307, 122.961, 113.8575]).astype(np.float32)[np.newaxis, :, np.newaxis, np.newaxis]), # Same as the transform in dataset
                 std=torch.tensor(np.array([51.5865, 50.847, 51.255]).astype(np.float32)[np.newaxis, :, np.newaxis, np.newaxis]), use_rs=False, loss='ce',
    ...
    x_adv = x_adv.clamp((0.0 * 255 - mean )/ std, (1.0 * 255 - mean) / std)
    x_best = x_adv.clone()
    ...
    if logits.detach().max(1)[1].size() != y.size():
        ...
        if True:  # with torch.no_grad()
            ...
            if norm == 'Linf':
                ...
                x_adv_1 = torch.clamp(torch.min(torch.max(x_adv_1,
                                                          x - eps * 255 / std), x + eps * 255 / std), (0.0 * 255 - mean )/ std, (1.0 * 255 - mean) / std)
