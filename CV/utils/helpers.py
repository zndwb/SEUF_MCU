...
def load_pretrained_pos_emb(model, cfg=None, num_classes=1000, in_chans=3, filter_fn=None, strict=True, pos_embed_interp=False, num_patches=576, align_corners=False, img_h=None, img_w=None):
    ...
    strict = False
    msg = model.load_state_dict(pos_emb_state_dict, strict=False)
...
def load_pretrained(model, cfg=None, num_classes=1000, in_chans=3, filter_fn=None, strict=True, pos_embed_interp=False, num_patches=576, align_corners=False, img_h=None, img_w=None):
    ...
    if 'patch_embed.proj.weight' in state_dict:
        ...
        current_weight = model.patch_embed.proj.weight
        if pretrained_weight.shape != current_weight.shape:
            print(f"Interpolating patch_embed weights from {pretrained_weight.shape} to {current_weight.shape}")
            # 插值调整卷积核尺寸
            interpolated_weight = torch.nn.functional.interpolate(
                pretrained_weight,
                size=current_weight.shape[-2:],  # 调整卷积核的高宽
                mode='bicubic',
                align_corners=False
            )
            # 替换为新权重
