...
class CurveModule(Module):
    def __init__(self, fix_points, parameter_names=(), layer_name=None):
        super(CurveModule, self).__init__()
        self.fix_points = fix_points.copy() if fix_points else []
    ...
    def set_trainable(self, trainable):
        ...
        if not trainable:
            self.fix_points = [True] * self.num_bends

    def compute_weights_t(self, coeffs_t):
        # Use original compute_weights_t logic but respect trainability
        if self.trainable:
            w_t = [None] * len(self.parameter_names)
            self.l2 = 0.0
            for i, parameter_name in enumerate(self.parameter_names):
            ...
            # If not trainable, use weights from first bend point
            w_t = [None] * len(self.parameter_names)
            for i, parameter_name in enumerate(self.parameter_names):
                parameter = getattr(self, '%s_%d' % (parameter_name, 0))
                if parameter is not None:
                    w_t[i] = parameter
            return w_t
...
#
#     def forward(self, input, t=None, quarter=False):
#         if t is None:
#             if quarter==False:
#                 t = input.data.new(1).uniform_()
#             else:
#                 t = input.data.new(1).uniform_(0.75, 1)
#         coeffs_t = self.coeff_layer(t)
