...
class QwenMoEEvaluator:
    """
    Evaluate Qwen1.5-MoE-A2.7B-Chat on:
      - MMLU accuracy
    ...
    def eval_mmlu(self) -> Dict[str, Any]:
        ...
        for sample in tqdm(test_ds, desc="MMLU(Qwen)", ncols=80):
    ...
    def eval_wmdp(self) -> Dict[str, Any]:
        ...
        for sample in tqdm(test_ds, desc="WMDP(Qwen)", ncols=80):
    ...
    def eval_rwku(self) -> Dict[str, Any]:
        ...
        for sample in tqdm(test_ds, desc="RWKU(Qwen)", ncols=80):
    ...
    def eval_all(self) -> Dict[str, Any]:
        ...
        print("=== Evaluating Qwen1.5-MoE-A2.7B-Chat on MMLU ===")
        results.update(self.eval_mmlu())

        print("\n=== Evaluating Qwen1.5-MoE-A2.7B-Chat on WMDP ===")
        results.update(self.eval_wmdp())

        print("\n=== Evaluating Qwen1.5-MoE-A2.7B-Chat on RWKU ===")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate Qwen1.5-MoE-A2.7B-Chat on MMLU, WMDP, and RWKU."
    )
    parser.add_argument("--model-name", type=str, default="Qwen/Qwen1.5-MoE-A2.7B-Chat")
    ...
    parser.add_argument("--output-json", type=str, default="eval_qwen_results.json")
    parser.add_argument(
        "--wmdp-dataset",
        type=str,
        default="cais/wmdp",
...
def main() -> None:
    ...
    evaluator = QwenMoEEvaluator(config)
    results = evaluator.eval_all()

    print("\n=== Final Results (Qwen1.5-MoE-A2.7B-Chat) ===")
